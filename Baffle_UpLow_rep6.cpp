...
/***** Purpose: To calculate the coefficients for the u equation. ******/
	...
	for (I = Istart; I <= Iend; I++) {
		...
		for (J = Jstart; J <= Jend; J++) {
			...
			if (i == PosBaffleLow && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
				
			
			if (i == PosBaffleLow + DistanceInbetween && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp + DistanceInbetween && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
				
				
			if (i == PosBaffleLow + DistanceInbetween*2 && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp + DistanceInbetween*2 && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
				
				
			if (i == PosBaffleLow + DistanceInbetween*3 && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp + DistanceInbetween*3 && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
				
			
			if (i == PosBaffleLow + DistanceInbetween*4 && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp + DistanceInbetween*4 && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
				
				
			if (i == PosBaffleLow + DistanceInbetween*5 && J < LengthBaffle)   // baffle Lower
				SP[i][J] = LARGE;
				
			if (i == PosBaffleUp + DistanceInbetween*5 && J > LengthBaffle)   // baffle Upper
				SP[i][J] = LARGE;
...
/***** Purpose: To calculate the coefficients for the v equation. ******/
	...
	for (I = Istart; I <= Iend; I++) {
		...
		for (J = Jstart; J <= Jend; J++) {
			...
			if (I == PosBaffleLow && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			
			if (I == PosBaffleLow + DistanceInbetween && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + DistanceInbetween + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp + DistanceInbetween && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + DistanceInbetween + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
				
			if (I == PosBaffleLow + DistanceInbetween*2 && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + DistanceInbetween*2 + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp + DistanceInbetween*2 && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + DistanceInbetween*2 + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
				
				
			if (I == PosBaffleLow + DistanceInbetween*3 && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + DistanceInbetween*3 + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp + DistanceInbetween*3 && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + DistanceInbetween*3 + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
				
				
			if (I == PosBaffleLow + DistanceInbetween*4 && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + DistanceInbetween*4 + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp + DistanceInbetween*4 && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + DistanceInbetween*4 + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
				
				
			if (I == PosBaffleLow + DistanceInbetween*5 && j < LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleLow + DistanceInbetween*5 + 1 && j < LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
				
			if (I == PosBaffleUp + DistanceInbetween*5 && j > LengthBaffle)       /* left of first baffle Lower */
				aE[I][j] = 0;

			if (I == PosBaffleUp + DistanceInbetween*5 + 1 && j > LengthBaffle)     /* right of first baffle Lower */
				aW[I][j] = 0;
...
/***** Purpose: Creating result table ******/
	if (time == Dt)
		printf ("iter\t Time\t \tSAVG\t Ios\t \t dp \t \tmean frac\n");

	printf ("%4d %10.3e\t%10.2e\t%10.2e\t%10.2e\t%10.2e\n", 
             iter, time, SAVG, IoS, dp, frac_end);
